...
// تبديل الاشتراك/إلغاء الاشتراك
  void toggleSubscription() async {
    if (isSubscribed) {
      ...
      final controlUserId = prefs.getString('control_user_id');
      if (controlUserId != null) {
        await FirebaseFirestore.instance.collection('controlUsers').doc(controlUserId).set({'lab_order_subscribed': false}, SetOptions(merge: true));
      } else {
      ...
      final controlUserId = prefs.getString('control_user_id');
      if (controlUserId != null) {
        await FirebaseFirestore.instance.collection('controlUsers').doc(controlUserId).set({'lab_order_subscribed': true}, SetOptions(merge: true));
      } else {
