...
class LabLocationScreen extends StatefulWidget {
  ...
  final String labId;
  const LabLocationScreen({super.key, required this.labName , required this.labId});
...
class _LabLocationScreenState extends State<LabLocationScreen> {
  ...
  Future<void> _saveLocation() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('labToLap').doc(widget.labId).set({
        "location": {
                    ...
                    Expanded(
                      child: GoogleMap(
                        initialCameraPosition: CameraPosition(
                          target: selectedLocation ?? const LatLng(15.5007, 32.5599),
                          zoom: 12,
                        ),
