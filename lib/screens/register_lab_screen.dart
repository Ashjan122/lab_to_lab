    ...
    try {
      ...
      if (nameDup.docs.isNotEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('اسم المعمل مستخدم مسبقاً'),
      ...
      if (userDup.docs.isNotEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('اسم المستخدم مستخدم مسبقاً'),
              backgroundColor: Colors.red,
            ),
          );
        }
...
رقم الهاتف: $phoneRaw''';
  ...
  Future<void> _sendWelcomeSms({
    ...
    final String url =
        '${baseUrl}?username=${username}&password=${Uri.encodeComponent(password)}&phone_number=${formatted}&message=${encodedText}&sender=${encodedSender}';
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception(
          'Airtel HTML SMS failed: ${response.statusCode} ${response.body}',
        );
      }
    } catch (e) {
      // ignore: avoid_print
